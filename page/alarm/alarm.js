...
window.onload=()=>{
	...
	let alarmDb= new AlarmDatabase();
	alarmDb.load(()=>{
		alarmDb.getAllAlarms((aPlans)=>{
...
function clickPlan(aData){
	if(aData=="+"){
		window.parent.openPage("editAlarm/editAlarm.html?id=+")
	}else{
		window.parent.openPage("editAlarm/editAlarm.html?id="+aData._id)
	}
}
